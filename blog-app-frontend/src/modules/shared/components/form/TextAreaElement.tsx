...
export default function TextAreaElement({
  ...
  return (
    <div className="flex w-full flex-col items-start gap-1">
      ...
      <textarea
        className={`${className || ""} ${errorMessage ? "border-error" : ""} p-1.5 mb-0.5 min-h-24 rounded-lg w-full max-w-lg border text-xs sm:text-sm md:text-base xl:text-lg md:min-h-28 lg:min-h-32`}
        id={name}
        name={name}
        {...props}
      ></textarea>
